...
:::warning
This rule is being split into [`no-restricted-types`](./no-restricted-types.mdx), [`no-uppercase-alias-types`](./no-uppercase-alias-types.mdx), and (coming in v8) [`no-empty-object-type`](https://v8--typescript-eslint.netlify.app/rules/no-empty-object-type).

We've found that `ban-types` serves two purposes: banning some dangerous built-in types _and_ allowing users to ban their own typeAlias.
The new `no-empty-object-type` and `no-uppercase-alias-types` rules are more precise and user-friendly for banning the dangerous built-in types.

The current name `no-ban-types` will be deprecated in version 8 of typescript-eslint, and removed in a future major version.
:::
...
- [`no-empty-object-type`](https://v8--typescript-eslint.netlify.app/rules/no-empty-object-type/) (in typescript-eslint@8)
- [`no-uppercase-alias-types`](./no-uppercase-alias-types.mdx)
